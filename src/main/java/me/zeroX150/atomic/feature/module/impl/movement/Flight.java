...
public class Flight extends Module {
    ...
    final BooleanValue bypassVanillaAc = (BooleanValue) this.config.create("Vanilla AC bypass", true).description("Whether or not to bypass the vanilla Anticheat (Flying is not enabled on this server)");
    ...
    public void tick() {
        ...
        switch (mode.getValue().toLowerCase()) {
            ...
            case "jetpack":
                ...
                break;
            case "3d":
                Atomic.client.player.setVelocity(Atomic.client.player.getRotationVector().multiply(speed).multiply(Atomic.client.player.input.pressingForward ? 1 : (Atomic.client.player.input.pressingBack ? -1 : 0)));
                break;
        }
    }
