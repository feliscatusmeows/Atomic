...
public class BetterCrosshair extends Module {
    ...
    public static void render() {
        ...
        if (gradient.getValue()) {
            Color start = Renderer.modify(color.getColor(), -1, -1, -1, transparentStart.getValue() ? 0 : -1);
            Color end = Renderer.modify(gradientEnd.getColor(), -1, -1, -1, transparentEnd.getValue() ? 0 : -1);
            Renderer.gradientLineScreen(start, end, x + i, y, x + i + l, y);
            Renderer.gradientLineScreen(start, end, x, y + i, x, y + i + l);
            Renderer.gradientLineScreen(start, end, x - i, y, x - i - l, y);
            Renderer.gradientLineScreen(start, end, x, y - i, x, y - i - l);
        } else {
            Renderer.lineScreenD(color.getColor(), x + i - .5, y, x + i + l - .5, y);
            Renderer.lineScreenD(color.getColor(), x, y + i, x, y + i + l);
            Renderer.lineScreenD(color.getColor(), x - i, y, x - i - l, y);
            Renderer.lineScreenD(color.getColor(), x, y - i + .5, x, y - i - l + .5);
        }
    }
